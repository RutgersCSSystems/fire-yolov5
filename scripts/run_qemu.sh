...
sudo qemu-system-x86_64 -kernel $KERNEL/vmlinuz-$VER -hda $QEMU_IMG_FILE -append "root=/dev/sda rw" --enable-kvm -m $QEMUMEM -numa node,nodeid=0,cpus=0-7,mem=25G -numa node,nodeid=1,cpus=16-23,mem=54G -smp sockets=2,cores=2,threads=2,maxcpus=16 -curses -net user,hostfwd=tcp::10000-:22
