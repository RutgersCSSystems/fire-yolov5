...
sudo qemu-system-x86_64 -kernel $KERNEL/vmlinuz-$VER -hda $QEMU_IMG_FILE -append "root=/dev/sda rw" --enable-kvm -m $QEMUMEM -numa node,nodeid=0,cpus=0-8 -numa node,nodeid=1,cpus=9-15 -smp sockets=2,cores=2,threads=2,maxcpus=16 -curses -net user,hostfwd=tcp:127.0.0.1:2222-:22   #-netdev tap,id=mynet0,ifname=eth0 -device e1000,netdev=mynet0
