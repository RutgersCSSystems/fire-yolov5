...
def main(argv):
	...
	ctype = 'eps'
	canvas= make_canvas(ctype, title=outputfile, dimensions=[600,280])
	...
	d = drawable(canvas,  coord=[90,60], xrange=[-0.5,t.getmax('rownumber')+1], yrange=[0,ymax])
	...
	#axis(d, xtitle=xlegend, xmanual=t.query(select='Threads,rownumber'),
	 #    ytitle='Memory (MB)', yauto=[0,ymax,yint], ytitlesize = 20, xtitlesize = 10)

	axis(d, xtitle=xlegend, xmanual=t.query(select='ops,rownumber'),
	     ytitle='Memory (MB)', yauto=[0,ymax,yint], ylabelfontsize=20, xlabelfontsize=20, 
	     ytitlesize = 20, xtitlesize = 20)
	...
	# randread bar
	barargs['yfield'] = 'devfs_inode'
	barargs['legendtext'] = 'inode'
	barargs['fillcolor'] = 'red'
	p.verticalbars(**barargs)

	# network bar
	barargs['stackfields'].append(barargs['yfield'])
	barargs['yfield'] = 'fileptrs'
	barargs['legendtext'] = 'fileptrs'
	...
	# storage bar
	barargs['stackfields'].append(barargs['yfield'])
	barargs['yfield'] = 'IO_queue_cmd'
	barargs['legendtext'] = 'IO queue'
	barargs['fillcolor'] = 'yellow'
	p.verticalbars(**barargs)

	#barargs['stackfields'].append(barargs['yfield'])
	#barargs['yfield'] = 'IO_queue_data'
	#barargs['legendtext'] = 'IO queue data'
	...
	# between legend symbols (not considering text) with skipspace.
        L.draw(canvas, coord=[d.left()+20, d.top()-5], skipspace=110, skipnext=1, fontsize=20.0)
