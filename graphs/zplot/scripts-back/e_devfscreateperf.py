...
def main(argv):
	...
	ctype = 'eps'
	c1= canvas(ctype, title=outputfile, dimensions=[750,280])
	...
	d = drawable(c1,  coord=[90,60], xrange=[-0.5,t.getmax('rownumber')+1], yrange=[0,ymax])
	...
	#axis(d, xtitle=xlegend, xmanual=t.query(select='Threads,rownumber'),
	 #    ytitle='KOPS/sec', yauto=[0,ymax,yint], ytitlesize = 20, xtitlesize = 10)

	axis(d, xtitle=xlegend, xmanual=t.query(select='ops,rownumber'),
	     ytitle='KOPS/sec', yauto=[0,ymax,yint], ylabelfontsize=20, xlabelfontsize=20, 
	...
	# between legend symbols (not considering text) with skipspace.
        L.draw(c1, coord=[d.left()+20, d.top()-5], skipspace=110, skipnext=1, fontsize=20.0)
