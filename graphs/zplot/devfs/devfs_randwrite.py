...
def main(argv):
	...
	ctype = 'eps'
	canvas= make_canvas(ctype, title=outputfile, dimensions=[850,300])
	...
	# half bar width (0.3) to the left and right of the bar locations
	# so we don't overflow the drawable.
	d = drawable(canvas,  coord=[80,60], xrange=[-0.5,t.getmax('rownumber')+1], yrange=[0,ymax])
	...
	axis(d, xtitle=xlegend, xmanual=t.query(select='filesystem,rownumber'), xtitlesize=20, ytitlesize=20,
	     ytitle='Throughput (GB/s)', yauto=[0,ymax,yint], ylabelfontsize=20, xlabelfontsize=18)
	...
	barargs = {'drawable':d, 'table':t, 'xfield':'rownumber',
		   'linewidth':0, 'fill':True, 'barwidth':0.4,
		   'legend':L, 'stackfields':[]}

	# randread bar
	barargs['yfield'] = 'randwrite'
	barargs['legendtext'] = 'Random 4K-write'
	...
	# between legend symbols (not considering text) with skipspace.
        L.draw(canvas, coord=[d.left()+5, d.top()-5], skipspace=50, skipnext=1, fontsize=20.0)
