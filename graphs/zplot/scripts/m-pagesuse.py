...
def main(argv):
	...
	d = drawable(c1,  coord=xycord, xrange=[-0.5,t.getmax('rownumber')+1], yrange=[0,1800], dimensions=xystart)
	...
	axis(drawable=d, style='xy', xmanual=[['DevFS-cap',-0.4],['DevFS-demand',0.7],['DevFS-cache-dentry',1.7],['DevFS-cache-dentry-inode',2.7]], 
	     xlabelrotate=45, xlabelshift=[-20,-35],
	     ymanual=[['0',0],['400',400],['800', 800],['1200', 1200],['1600', 1600]],
	     #yauto=[0,ymax,yint],
	     ytitle='Memory (MB)', ylabelfontsize=yfont, xlabelfontsize=xfont,	
	     linewidth=0.5, ytitleshift	= [+7,0],
	...
	barargs['stackfields'].append(barargs['yfield'])
	barargs['yfield'] = 'others'
	barargs['legendtext'] = 'filemap'
	...
	# between legend symbols (not considering text) with skipspace.
        L.draw(c1, coord=xylegend, skipspace=50, skipnext=1, fontsize=xfont)
