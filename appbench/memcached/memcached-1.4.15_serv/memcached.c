...
#define _USE_MIGRATION

#ifdef _USE_MIGRATION
#include <migration.h>
#endif
...
void *mycalloc(size_t unit, size_t size){

	//total_sz += size;
	//fprintf(stdout,"mymalloc %zu total alloc %zu\n", size, total_sz);
#ifdef _USE_DIRECTED_ALLOC
    return xxmalloc(unit * size);
#else
	return malloc(unit*size);
#endif
}
...
static int add_msghdr(conn *c)
{
    ...
    if (c->msgsize == c->msgused) {
        msg = myrealloc(c->msglist, c->msgsize * 2 * sizeof(struct msghdr));
        if (! msg)
            return -1;
        c->msglist = msg;
        c->msgsize *= 2;
...
static void conn_init(void) {
    freetotal = 200;
    freecurr = 0;
    if ((freeconns = mycalloc(freetotal, sizeof(conn *))) == NULL) {
...
bool conn_add_to_freelist(conn *c) {
    ...
    if (freecurr < freetotal) {
        ...
        conn **new_freeconns = myrealloc(freeconns, sizeof(conn *) * newsize);
        if (new_freeconns) {
            freetotal = newsize;
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {
        if (!(c = (conn *)mycalloc(1, sizeof(conn)))) {
            fprintf(stderr, "mycalloc()\n");
            return NULL;
        }
        ...
        c->rbuf = (char *)mymalloc((size_t)c->rsize);
        c->wbuf = (char *)mymalloc((size_t)c->wsize);
        c->ilist = (item **)mymalloc(sizeof(item *) * c->isize);
        c->suffixlist = (char **)mymalloc(sizeof(char *) * c->suffixsize);
        c->iov = (struct iovec *)mymalloc(sizeof(struct iovec) * c->iovsize);
        c->msglist = (struct msghdr *)mymalloc(sizeof(struct msghdr) * c->msgsize);
...
static void conn_shrink(conn *c) {
    ...
    if (c->rsize > READ_BUFFER_HIGHWAT && c->rbytes < DATA_BUFFER_SIZE) {
        ...
        newbuf = (char *)myrealloc((void *)c->rbuf, DATA_BUFFER_SIZE);
    ...
    if (c->isize > ITEM_LIST_HIGHWAT) {
        item **newbuf = (item**) myrealloc((void *)c->ilist, ITEM_LIST_INITIAL * sizeof(c->ilist[0]));
        if (newbuf) {
            c->ilist = newbuf;
            c->isize = ITEM_LIST_INITIAL;
        }
    ...
    if (c->msgsize > MSG_LIST_HIGHWAT) {
        struct msghdr *newbuf = (struct msghdr *) myrealloc((void *)c->msglist, MSG_LIST_INITIAL * sizeof(c->msglist[0]));
        if (newbuf) {
            c->msglist = newbuf;
            c->msgsize = MSG_LIST_INITIAL;
        }
    ...
    if (c->iovsize > IOV_LIST_HIGHWAT) {
        struct iovec *newbuf = (struct iovec *) myrealloc((void *)c->iov, IOV_LIST_INITIAL * sizeof(c->iov[0]));
        if (newbuf) {
            c->iov = newbuf;
            c->iovsize = IOV_LIST_INITIAL;
        }
...
static int ensure_iov_space(conn *c) {
    ...
    if (c->iovused >= c->iovsize) {
        int i, iovnum;
        struct iovec *new_iov = (struct iovec *)myrealloc(c->iov,
...
static int build_udp_headers(conn *c) {
    ...
    if (c->msgused > c->hdrsize) {
        void *new_hdrbuf;
        if (c->hdrbuf)
            new_hdrbuf = myrealloc(c->hdrbuf, c->msgused * 2 * UDP_HEADER_SIZE);
        else
            new_hdrbuf = mymalloc(c->msgused * 2 * UDP_HEADER_SIZE);
        if (! new_hdrbuf)
            return -1;
...
static bool grow_stats_buf(conn *c, size_t needed) {
    ...
    if (nsize != c->stats.size) {
        char *ptr = myrealloc(c->stats.buffer, nsize);
        if (ptr) {
            c->stats.buffer = ptr;
            c->stats.size = nsize;
        } else {
...
static void bin_read_key(conn *c, enum bin_substates next_substate, int extra) {
    ...
    if (c->rlbytes > c->rsize - offset) {
        ...
        if (nsize != c->rsize) {
            ...
            char *newm = myrealloc(c->rbuf, nsize);
            if (newm == NULL) {
                if (settings.verbose) {
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
    ...
    do {
        while(key_token->length != 0) {
            ...
            if (it) {
                if (i >= c->isize) {
                    item **new_list = myrealloc(c->ilist, sizeof(item *) * c->isize * 2);
                    if (new_list) {
                        c->isize *= 2;
                        c->ilist = new_list;
                    } else {
                ...
                if (return_cas)
                {
                  ...
                  if (i >= c->suffixsize) {
                    char **new_suffix_list = myrealloc(c->suffixlist,
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        if (c->rbytes >= c->rsize) {
            ...
            ++num_allocs;
            char *new_rbuf = myrealloc(c->rbuf, c->rsize * 2);
            if (!new_rbuf) {
