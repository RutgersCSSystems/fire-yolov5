...
static int ms_conn_udp_init(ms_conn_t *c, const bool is_udp)
{
  ...
  {
    c->rudpbuf= (char *)mymalloc((size_t)c->rudpsize);
    c->udppkt= (ms_udppkt_t *)mymalloc(MAX_UDP_PACKET * sizeof(ms_udppkt_t));
                        ...
                        const bool is_udp)
{
  ...
  c->iov= (struct iovec *)mymalloc(sizeof(struct iovec) * (size_t)c->iovsize);
  c->msglist= (struct msghdr *)mymalloc(
  ...
  c->tcpsfd= (int *)mymalloc((size_t)c->total_sfds * sizeof(int));
...
static int ms_item_win_init(ms_conn_t *c)
{
  ...
  c->item_win= (ms_task_item_t *)mymalloc(
...
static int ms_udp_read(ms_conn_t *c, char *buf, int len)
{
  ...
  while (1)
  {
    ...
    {
      char *new_rbuf= myrealloc(c->rudpbuf, (size_t)c->rudpsize * 2);
      if (! new_rbuf)
      {
...
static int ms_try_read_network(ms_conn_t *c)
{
  ...
  while (1)
  {
    if (c->rbytes >= c->rsize)
    {
      char *new_rbuf= myrealloc(c->rbuf, (size_t)c->rsize * 2);
      if (! new_rbuf)
      {
        fprintf(stderr, "Couldn't realloc input buffer.\n");
...
static int ms_add_msghdr(ms_conn_t *c)
{
  ...
  {
    msg=
      myrealloc(c->msglist, (size_t)c->msgsize * 2 * sizeof(struct msghdr));
    if (! msg)
      return -1;
...
static int ms_build_udp_headers(ms_conn_t *c)
{
  ...
  if (c->msgused > c->hdrsize)
  {
    void *new_hdrbuf;
    if (c->hdrbuf)
      new_hdrbuf= myrealloc(c->hdrbuf,
                          (size_t)c->msgused * 2 * UDP_HEADER_SIZE);
    else
      new_hdrbuf= mymalloc((size_t)c->msgused * 2 * UDP_HEADER_SIZE);
    if (! new_hdrbuf)
      return -1;
...
static void ms_conn_shrink(ms_conn_t *c)
{
  ...
  {
    char *new_rbuf= (char *)myrealloc(c->rudpbuf, (size_t)c->rudpsize * 2);
    if (! new_rbuf)
    {
      c->rudpbuf= new_rbuf;
      c->rudpsize= UDP_DATA_BUFFER_SIZE;
  ...
  {
    struct msghdr *newbuf= (struct msghdr *)myrealloc(
      (void *)c->msglist,
      MSG_LIST_INITIAL
      * sizeof(c->msglist[0]));
    if (newbuf)
  ...
  {
    struct iovec *newbuf= (struct iovec *)myrealloc((void *)c->iov,
                                                  IOV_LIST_INITIAL
                                                  * sizeof(c->iov[0]));
    if (newbuf)
    {
