...
##For a given IOR configuration, plot bandwidth and runtime wrt prediction and nproc
...
BW_PLOT_FILE=${OUTFOLDER}/$TODAY-$APP-BW-seqread-diff-file-${SETRA}_setra-120G_fsize-1M_tsize.dat
RT_PLOT_FILE=${OUTFOLDER}/$TODAY-$APP-RT-seqread-diff-file-${SETRA}_setra-120G_fsize-1M_tsize.dat
...
for NPROC in "${nproc[@]}" #For each setra size
do
