...
def create_dataloader(
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(
            ...
            cache_control_pipe=cache_control_pipe
        )
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        self.cache_control_pipe = cache_control_pipe

        # start cache control pipe here
        if self.cache_control_pipe:
            self.start_pipe_thread(self.cache_control_pipe)
        ...
        if cache_images == "ram" and not self.check_cache_ram(prefix=prefix, cache_size=cache_size):
            cache_images = False
        self.ims = [None] * n
        ...
        if cache_images:
            ...
            for i, x in pbar:
                if cache_images == "disk":
                    ...
                    curr_img_size = self.ims[i].nbytes * WORLD_SIZE
                    if self.cache_size and b + curr_img_size > (self.cache_size * gb):
                        LOGGER.info(f'Cache full after {i} images ({self.cache_size:.1f}GB), stopping caching')
                        break
                    b += curr_img_size
                if self.cache_size:
                    pbar.desc = f"{prefix}Caching images ({b / gb:.1f}GB {cache_images}) (Cache size: {cache_size:.1f} GB)"
                else:
                    pbar.desc = f"{prefix}Caching images ({b / gb:.1f}GB {cache_images})"
            pbar.close()

    # Cache control pipe
    def process_cache_command(self, command):
        # Test command
        if command == "ping":
            print("\npong")
            return True
        # Clear cache command
        if command == "clear":
            # set all elements in self.ims to None
            old_len = len(self.ims)
            del self.ims
            del self.im_hw0
            del self.im_hw
            self.ims = [None] * old_len
            self.im_hw0 = [None] * old_len
            self.im_hw = [None] * old_len
            collected_objects = gc.collect()
            print(f"\nCache cleared. Garbage collector: collected {collected_objects} objects.\n")
            return True
        print(f"\n unknown command {command}. Valid commands: ping, clear\n")
        return False

    def read_control_pip_non_blocking(self, control_pipe):
        # Open the pipe in non-blocking mode
        with open(control_pipe, 'r', buffering=1) as pipe:
            while True:
                try:
                    data = pipe.readline()
                    if data:  # Only process if there's data ending with a newline
                        self.process_cache_command(data.strip())
                except IOError as e:
                    # EAGAIN and EWOULDBLOCK are errors for non-blocking operations
                    # that indicate no data is available
                    if e.errno != errno.EAGAIN and e.errno != errno.EWOULDBLOCK:
                        raise  # Reraise errors unrelated to non-blocking reads

    def start_pipe_thread(self, pipe_name):
        # Create pipe if it doesn't exist
        if not os.path.exists(pipe_name):
            os.mkfifo(pipe_name)
        thread = threading.Thread(target=self.read_control_pip_non_blocking, args=(pipe_name,))
        thread.daemon = True  # Daemonize thread
        thread.start()
        print("Pipe control thread started")

    def check_cache_ram(self, safety_margin=0.1, prefix="", cache_size=None):
        ...
        # Use the determined cache size if it is provided.
        # It shouldn't exceed the available memory.
        if cache_size:
            mem_required = min(cache_size * gb, mem_required)
        cache = mem_required * (1 + safety_margin) < mem_avail  # to cache or not to cache, that is the question
        if not cache:
            LOGGER.info(
                f'{prefix}{mem_required / gb:.1f}GB RAM required for cache, '
