...
one_dat_file = "nobg_v_bg_oneanalysis.csv"
...
def write_from_dict(filepath, para_dict_1, para_dict_2):
    line = ""
    for s in variants:
...
def main():
    ...
    for workload in WORKLOADS:
        first_time = True
        para_dict_1["workload"] = workload
        para_dict_2["workload"] = workload
        ...
        for tup_inv in iter_variants: #For each permutation 
            for i in range(len(variants)): #Populate para_dict
                para_dict_1[variants[i]] = tup_inv[i]
                para_dict_2[variants[i]] = tup_inv[i]

            if first_time == True: ##Write first line to outfile
                write_dat(folder_out+one_dat_file, ",".join(out_order))
                first_time = False

            in_filename = get_infilename(para_dict_1)
            for dat in data: # For each data types    
                data_val_1 = Extract(in_folder1+in_filename, dat)
                para_dict_1[dat] = str(data_val_1)
                
                data_val_2 = Extract(in_folder2+in_filename, dat)
                para_dict_2[dat] = str(data_val_2)
            
            write_from_dict(folder_out+one_dat_file, para_dict_1, para_dict_2)
