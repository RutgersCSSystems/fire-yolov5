...
GEN_RESULT_PATH() {
	...
	then 
		...
		RESULTS=$OUTPUTDIR/$APPOUTPUTNAME/$KEYCOUNT"M-KEYS"/$THREAD/"SNAPPYTHREADS-"$BATCHSIZE/$WORKLOAD/
		mkdir -p $RESULTS
...
GEN_RESULT_PATH_SNAPPY() {
	...
	RESULTFILE=""
        RESULTS=$OUTPUTDIR/$APPOUTPUTNAME/$KEYCOUNT"M-KEYS"/$THREAD/"SNAPPYTHREADS-"$BATCHSIZE/$WORKLOAD/
	mkdir -p $RESULTS
	SNAPPY_RESULTFILE=$RESULTS/"SNAPPYOUT-"$CONFIG.out
}
				...
				do
					...
					do
						...
						do
							...
							CLEAR_PROCESS
							RESULTS=""
							GEN_RESULT_PATH_SNAPPY $WORKLOAD $CONFIG $THREAD $NUM
							#echo $RESULTFILE
							if [ "$CONFIG" != "isolated" ]; then
								echo $SNAPPY_RESULTFILE
								RUN_SNAPPY $SNAPPY_RESULTFILE $BATCHSIZE
							fi
							cd $DBHOME
