...
GEN_RESULT_PATH() {
	...
	let KEYCOUNT=$NUM/1000000
        ...
        then
		    RESULTS=$OUTPUTDIR"-"$G_TRIAL/"REMOTE"/$APPOUTPUTNAME/$KEYCOUNT"M-KEYS"/$WORKLOAD/$THREAD
		    mkdir -p $RESULTS
	    ...
	    then 
        	...
        	RESULTS=$OUTPUTDIR"-"$G_TRIAL/"REMOTE"/$APPOUTPUTNAME/$KEYCOUNT"M-KEYS"/"MEMFRAC"$MEM_REDUCE_FRAC/$WORKLOAD/$THREAD/
		    mkdir -p $RESULTS
		    RESULTFILE=$RESULTS/$CONFIG".out"
	fi
