...
#config_arr = ["isolated"]  # Updated order
#config_out_arr = ["isolated"]  # Updated order
...
def main():
    with open(output_file_throughput, mode='w', newline='') as csv_file, \
        ...
        header_row_throughput = ["Workload"] + [f"{config}_{memfrac}" for memfrac in memfrac_arr for config in config_out_arr]
        csv_writer.writerow(header_row_throughput)
        
        # Write the header row with column names for energy
        header_row_energy = ["Configuration", "MEMFRAC", "CPU-POWER", "DRAM-POWER"]
        ...
        for workload in workload_arr:
            for memfrac in memfrac_arr:
                ...
                for config, config_out in zip(config_arr, config_out_arr):
                    ...
                    #20M-KEYS/MEMFRAC3/32/batchsize-40/
                    #/users/kannan11/ssd/ioopt/results/HOTCARBON-FINAL-MEMFRAC/WILDFIRE/ROCKSDB/20M-KEYS/MEMFRAC4/32/batchsize-40/multireadrandom//isolated.out
                    file_path = os.path.join(base_dir, f"MEMFRAC{memfrac}", thread_arr[0],  "batchsize-40", workload,  f"{config}.out")
                    print(file_path)
                    if os.path.exists(file_path):
                        with open(file_path, 'r') as file:
                            ...
                            if cpu_energy is not None:  # Write energy data if available
                                csv_energy_writer.writerow([config_out, memfrac, total_energy['CPU'], total_energy['DRAM']])
                    else:
                        ...
                        csv_energy_writer.writerow([config_out, memfrac, 0, 0])  # Default energy values if file not found
