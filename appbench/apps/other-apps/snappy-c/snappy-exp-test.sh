#!/bin/bash
set -x
DBHOME=$PWD
THREAD=4
VALUE_SIZE=4096
SYNC=0
KEYSIZE=1000
WRITE_BUFF_SIZE=67108864

WORKLOAD="snappy-threads"
APPPREFIX="/usr/bin/time -v"

APP="snappy"
APPOUTPUTNAME="snappy"

RESULTS="RESULTS"/$WORKLOAD
RESULTFILE=""

DBDIR=$SHARED_DATA/snappy
RESULTDIR=$SHARED_DATA/$APP/
FILECOUNT=100

mkdir -p $RESULTS

NUM=2000
declare -a num_arr=("10000")
#declare -a workload_arr=("readrandom" "readseq" "readreverse" "compact" "overwrite" "readwhilewriting" "readwhilescanning")
#declare -a thread_arr=("4" "8" "16" "32")
#declare -a config_arr=("Vanilla" "Cross_Naive" "CPBI" "CNI" "CPBV" "CPNV" "CPNI")

declare -a thread_arr=("8")
declare -a workload_arr=("500")
declare -a config_arr=("Cross_Naive" "CPBI" "CNI" "CPBV" "CPNV" "CPNI")


FlushDisk()
{
        sudo sh -c "echo 3 > /proc/sys/vm/drop_caches"
        sudo sh -c "sync"
        sudo sh -c "echo 3 > /proc/sys/vm/drop_caches"
        sudo sh -c "sync"
        #sudo dmesg --clear
        sleep 2
}

CLEAR_DATA()
{
        sudo killall $APP
        sudo killall $APP
        sleep 3
        sudo killall $APP
        rm -rf $DBDIR/OUT*
}


GENDATA() {

        filesize=$1
	numfiles=$2
        threads=$3


        let file_perthread=$numfiles/$threads
        echo $file_perthread

        for (( t=1; t<=$threads; t++ ))
        do
                DIR=$DBDIR/$t

                mkdir -p $DIR
		cd $DIR
                echo $numfiles
                rm *
		cd $DBHOME
                $DBHOME/gen_file_posix $numfiles $filesize $DIR
        done
}



COMPILE_AND_WRITE()
{

        echo "..........BEGIN DATA GENERATION..............."
        export LD_PRELOAD=""
	mkdir -p $RESULTS

        filesize=$1
	numfiles=$2
        threads=$3

	cd $PREDICT_LIB_DIR
	$PREDICT_LIB_DIR/compile.sh &> compile.out
	cd $DBHOME

	#Generate data
	GENDATA $filesize $numfiles $threads
	sudo dmesg -c &> del.txt
	rm del.txt

        echo "..........END DATA GENERATION..............."
}

GEN_RESULT_PATH() {
	WORKPATH=$1
	CONFIG=$2
	THREAD=$3
	#WORKLOAD="DUMMY"
	#RESULTFILE=""
        RESULTS=$OUTPUTDIR/$APPOUTPUTNAME/$WORKPATH/$THREAD
	mkdir -p $RESULTS
	RESULTFILE=$RESULTS/$CONFIG.out
}



RUN() {

	#NUM represents file size in bytes
	for NUM in "${num_arr[@]}"
	do
		for THREAD in "${thread_arr[@]}"
		do
			PARAMS="$DBDIR $THREAD"

			#NUMFILES
			for WORKLOAD in "${workload_arr[@]}"
			do
				#COMPILE_AND_WRITE $NUM $WORKLOAD $THREAD

				for CONFIG in "${config_arr[@]}"
				do
					RESULTS=""
					GEN_RESULT_PATH "files-"$WORKLOAD $CONFIG $THREAD

					mkdir -p $RESULTS
					#echo $RESULTFILE
					#continue

					echo "RUNNING $CONFIG and writing results to #$RESULTS/$CONFIG.out"
					echo "..................................................."
					#export LD_PRELOAD=/usr/lib/lib_$CONFIG.so
					echo "$APPPREFIX $DBHOME/$APP $PARAMS"
					#$APPPREFIX 
					LD_PRELOAD=/usr/lib/lib_$CONFIG.so $DBHOME/$APP $PARAMS  &> $RESULTFILE
					export LD_PRELOAD=""
					sudo dmesg -c &>> $RESULTFILE
					echo ".......FINISHING $CONFIG......................"
					FlushDisk
					CLEAR_DATA
				done
			done
		done
	done
}

RUN
#CLEAR_DATA
exit

