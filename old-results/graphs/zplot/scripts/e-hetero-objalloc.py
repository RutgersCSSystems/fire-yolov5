...
def main(argv):
        ...
        #xlegend='#. of files'
        xlegend='      DevFS-J               all_kernel                       page_cache'
        xfont = 17
        yfont = 17
        xdim = 650
        ydim = 280
	...
	page_cache = [row[0] for row in data]
	kernel_buffer = [row[1] for row in data]
	all_kernel = [row[2] for row in data]
	obj_affinity = [row[3] for row in data]
	FS = ['page_cache', 'kernel_buffer', 'all_kernel', 'obj_affinity']
	...
	rects0 = plt.bar(index, page_cache, bar_width,
			 alpha=opacity,
			 color='#a9a9a9',                 
			 edgecolor = "grey",  
			 label=FS[0])

	rects1 = plt.bar(index+ bar_width, kernel_buffer, bar_width,
			 alpha=opacity,
			 color='gray',                 
			 edgecolor = "k",  
			 label=FS[1])

	rects2 = plt.bar(index+ 2*bar_width, all_kernel, bar_width,
			 alpha=opacity,
			 color='#000000',
			 edgecolor = "k",  
			 hatch='/',
	...
	rects3 = plt.bar(index+ 3*bar_width, obj_affinity, bar_width,
			 alpha=opacity,
			 color='#000000',
			 edgecolor = "k",  
			 label=FS[2])
	...
	plt.xticks(index + bar_width, ("Graphchi", "RocksDB", "fio", "Metis", "Redis", "Memcached", ))
	legend =plt.legend( (rects0[0], rects1[0], rects2[0], rects3[0]), ('page_cache', 'kernel_buffer', 'all_kernel', 'obj_affinity'))
