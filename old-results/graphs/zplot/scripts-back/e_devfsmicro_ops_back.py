...
def main(argv):
	...
	x = [None]*ngraphs
	y = [None]*ngraphs
	xlegend=['4K-Append','4K-Read','1MB-Append', '1MB-Read']
	...
	for tidx in range(len(inputfile)):
		...
		for btype in pattern:
		    ...
		    p.verticalbars(drawable=d[tidx], table=t[tidx], xfield='c0', yfield=colstr, fill=True,
				   fillcolor='darkgray', fillstyle=btype, barwidth=0.5,
	...
	# between legend symbols (not considering text) with skipspace.
        L.draw(canvas, coord=[d[0].left()+20, d[0].top()+20], skipspace=110, skipnext=1, fontsize=20.0)
