...
def main(argv):
	...
	x = [None]*ngraphs
	y = [None]*ngraphs
	xlegend=['create-files','reopen-files']
	...
	for tidx in range(len(inputfile)):
		...
		if tidx == 0:
			...
			axis(d[tidx], xtitle=xlegend[tidx], xtitleshift=[-30,0], xlabelshift=[-20,-10],
			     ytitle='Ops per sec(x1000)', yauto=[0,ymax,yint], ylabelfontsize=yfont, xlabelfontsize=xfont,
		...
		L = legend()
		legends = ['+cap','+cap +demand','+cap +demand +dentry','+cap +demand +dentry +inode']
		i = 0
		j = 1
		p = plotter()
	...
	# between legend symbols (not considering text) with skipspace.
        L.draw(canvas, coord=[d[0].left()+5, d[0].top()-20], skipspace=120, skipnext=1, fontsize=xfont)
